<script lang="ts">
  ...
  function submitForm() {
    ...
    if (!form.time) return alert('請選擇時段');
    ...
    if (form.delivery === '外送' && !form.address) {
      return alert('請輸入外送地址');
    }
...
<div class="min-h-screen flex flex-col items-center p-4 bg-[#F1E1FF]">
  ...
  <img src="/h.png" alt="Banner"
       class="w-full max-h-64 object-cover rounded-md mb-6">
  ...
  <div class="w-full max-w-md bg-white p-6 rounded-lg shadow-md space-y-5 text-xl">
    ...
    <div>
      ...
      <input type="text"
        ...
        placeholder="請輸入姓名"
        class="w-full border rounded-lg p-4 text-xl">
    </div>
    ...
    <div class="flex gap-6 items-center text-xl">
      ...
      <label class="flex items-center gap-2">
        <input type="radio" bind:group={form.gender} value="先生">
        先生
      </label>

      <label class="flex items-center gap-2">
        <input type="radio" bind:group={form.gender} value="小姐">
        小姐
      </label>
    </div>
    ...
    <div>
      ...
      <input type="text"
        ...
        placeholder="ex: 0916717797 或 02-26081202"
        class="w-full border rounded-lg p-4 text-xl">
    </div>
    ...
    <div class="flex items-center gap-2 w-full">
      ...
      <button
        ...
        class="bg-gray-300 text-2xl px-4 rounded-lg hover:bg-gray-400 h-14 w-14 flex items-center justify-center">
        −
      </button>
    ...
    <button
      on:click={submitForm}
      class="bg-blue-500 text-white px-6 py-4 rounded-lg hover:bg-blue-600 w-full">
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md space-y-4">
    ...
    <div class="flex flex-col sm:flex-row justify-center gap-3">
      ...
      <button
        ...
        class="bg-gray-300 px-6 py-3 rounded-lg hover:bg-gray-400 w-full sm:w-auto text-xl">
        修改
      </button>

      <button
        on:click={confirmOrder}
        class="bg-green-500 text-white px-6 py-3 rounded-lg hover:bg-green-600 w-full sm:w-auto text-xl">
        確認送出
      </button>
...
<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md text-center space-y-3 text-xl">
    ...
    <img src="/IMG_3119.jpeg"
         class="w-full max-h-96 object-contain rounded-md">

    <button
      on:click={() => showComplete.set(false)}
      class="bg-blue-500 text-white px-6 py-3 rounded-lg hover:bg-blue-600 w-full text-xl">
